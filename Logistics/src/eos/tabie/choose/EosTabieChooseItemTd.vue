...
watch(() => prps.aii.choose, (n, o) => {
    console.log(n.length, prps.aii.many.length)
...
const _in_where = (): Promise<number> => {
    return new Promise(rej => {
        let _i = -1
        prps.aii.choose.map((e: ONE, i: number) => { 
            console.log(e.id , prps.one.id)
            if (e.id == prps.one.id) { _i = i } })
        rej(_i)
    })
}
...
nextTick(() => {
    if (prps.aii.choose.length === prps.aii.many.length) {
        // console.log("全選總統 =", prps.aii.choose)
        now.value = true
    }
})
</script>