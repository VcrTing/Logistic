<template>
    <div class="py f-row">
        <eos-input class="w-25 w-333-p" :is_err="false" :header="'場表月份：'">
            <fn-time @resuit="(v: string) => form.date = v" ref="dateREF"/>
        </eos-input>
        <div class="w-56 w-666-p fx-s">
            ...
            <eos-input class="w-25 w-28-p" :is_err="false" :header="'後加單：'" :is_txt_mode="true">
                <div class="fx-l">
                    ...
                    <eos-switch @resuit="(v: boolean) => form.is_late_list = v" ref="iastREF"/>
...
const form = reactive(<ONE>{ list_id: '', remarks: '', date: '', is_late_list: false })
...
defineExpose({ 
    ...
    reset: (v: ONE) => { 
        ...
        iastREF.value.ioc(v.is_late_list)
        dateREF.value.ioc(v.date) 
    }     
})
</script>